...
// matchWildcard 模型映射的通配符匹配：* 匹配任意长度（含空）的字符序列，
// 可出现在模式任意位置且允许出现多个；模式不含 * 时退化为精确匹配。
// 例：claude-* 前缀、*-luna 后缀、gpt-*-luna 中缀、* 全匹配。
// 大小写敏感：模型名按原样比较，不做大小写归一。
func matchWildcard(pattern, str string) bool {
	if !strings.Contains(pattern, "*") {
		return pattern == str
	}
	// 贪心回溯：记住最近一个 * 的位置及它已吞掉的字符串下标，失配时让它多吞一个字符再试。
	patternIndex, strIndex := 0, 0
	...
	for strIndex < len(str) {
		...
		case patternIndex < len(pattern) && pattern[patternIndex] == '*':
			...
			patternIndex++
		case patternIndex < len(pattern) && pattern[patternIndex] == str[strIndex]:
			patternIndex++
			strIndex++
		case starIndex >= 0:
			starMatchIndex++
