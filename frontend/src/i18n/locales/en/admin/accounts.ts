    ...
    accounts: {
      ...
      cnProviders: {
        ...
        zhipuSpoof: {
          ...
          hint: 'Mimic the TLS fingerprint and session traits of real clients to reduce risk-control detection. Unrecognized clients are treated as not excluded and are spoofed as usual.',
          sessionMaskingHint: 'Keep the session ID of Anthropic-protocol requests fixed for 15 minutes so upstream sees one session',
          excludedTitle: 'Excluded platforms (selected client tools are NOT spoofed)',
          excludedHint: 'Only effective when the client User-Agent is recognized; unselected platforms are spoofed as usual.',
          platforms: {
            claudeCode: 'Claude Code',
            claudeIde: 'Claude for IDE',
            codex: 'Codex',
            opencode: 'OpenCode',
            crush: 'Crush',
            goose: 'Goose',
        ...
        balance: 'Balance --',
        clineModels: {
          title: 'Models & upstream providers',
          hint: 'Fetch the models this account can use and pick an upstream provider preference per model. Probing issues real upstream requests and consumes usage, so click only when needed.',
          fetch: 'Fetch models',
          fetching: 'Fetching...',
          fetchFailed: 'Failed to fetch models',
          needKey: 'Enter the API key first, or save the account and fetch afterwards.',
          notFetched: 'Models have not been fetched yet.',
          ...
          notProbed: 'Upstream providers not probed yet',
          modeLabel: 'Upstream preference',
          modeAuto: 'Auto',
          modeOnly: 'Only selected',
          modeOrder: 'Priority order',
          noChoice: 'No selectable providers (fixed upstream pipeline)',
          clearProviders: 'Clear',
          configured: 'Whitelisted & configured',
          whitelistBadge: 'whitelisted',
          whitelistAdd: 'Add to whitelist',
          whitelistRemove: 'Remove from whitelist',
          whitelistEmptyHint:
            'An empty whitelist keeps every upstream model available; adding models restricts the account to the whitelist.',
          catalog: 'Pay-as-you-go catalog',
          catalogShow: 'Expand',
          catalogHide: 'Collapse',
        ...
        windowMonthly: '30d',
        probe: 'Query',
        probeTooltip: 'Query the provider quota endpoint for 5-hour / weekly (OpenCode and Cline also monthly) rolling window usage',
      ...
      mapRequestModels:
        'Map request models to actual models. Left is the requested model, right is the actual model sent to API. Supports * wildcards anywhere in the pattern: claude-* (prefix), *-luna (suffix), gpt-*-luna (infix).',
      ...
      invalidModelPattern: 'Request model cannot be empty',
