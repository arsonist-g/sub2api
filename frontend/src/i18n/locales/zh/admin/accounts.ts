export default {
    accounts: {
      ...
      cnProviders: {
        ...
        zhipuSpoof: {
          title: '客户端伪装',
          hint: '模拟真实客户端的 TLS 指纹与会话特征，降低风控识别。识别不出的客户端按未排除处理，照常伪装。',
          sessionMaskingHint: '固定 15 分钟内 anthropic 协议请求的会话 ID，使上游视为同一会话',
          excludedTitle: '排除平台（选中的客户端工具不进行伪装）',
          excludedHint: '仅在能识别客户端 User-Agent 时生效；未选中的平台照常伪装。',
          platforms: {
        ...
        clineModels: {
          title: '模型与上游供应商',
          hint: '实时获取账号可用模型，并按模型指定上游供应商偏好。探测会向上游发起真实请求产生用量，仅在需要时点击。',
          fetch: '获取模型',
          fetching: '获取中...',
          fetchFailed: '获取模型失败',
          needKey: '请先填写 API Key，保存账号后再获取模型。',
          notFetched: '尚未获取模型清单。',
          ...
          modeOnly: '仅用所选',
          modeOrder: '优先顺序',
          noChoice: '无可选供应商（上游固定管道）',
          clearProviders: '清空',
          configured: '白名单与已配置模型',
          whitelistBadge: '白名单',
          whitelistAdd: '加入白名单',
          whitelistRemove: '移出白名单',
          whitelistEmptyHint:
            '白名单为空时该账号对上游全部模型开放；加入白名单后仅放行白名单内的模型。',
          catalog: '按量目录',
          catalogShow: '展开',
          catalogHide: '收起',
        ...
        windowMonthly: '30d',
        probe: '查询',
        probeTooltip: '请求供应商额度端点，查询 5 小时 / 每周（OpenCode / Cline 另有每月）滚动窗口用量',
      ...
      mapRequestModels: '将请求模型映射到实际模型。左边是请求的模型，右边是发送到 API 的实际模型。支持 * 通配符，可位于任意位置：claude-* 匹配前缀、*-luna 匹配后缀、gpt-*-luna 匹配中缀。',
